...
export default function FeedList({ agitId }) {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR(`agits/${agitId}/feeds?page=${page}`, async () => await getFeeds(agitId, page));

  const fetchData = async () => {
    if (isLoading) return;

    if (page == 0) setFeeds([...feedData.data]);
    else setFeeds((prev) => [...prev, ...feedData.data]);
  };

  const loadMore = () => {
    if (feedData?.hasNext && !isLoading) {
      setPage((prev) => prev + 1);
      mutate();
      fetchData();
    }
